...
export function buildScreenViewModel(
  ...
  if (snapshot.screen === "MENU") {
    return {
      ...
      title: "Stage Call",
      subtitle:
        "Route artists fast in timed rounds, keep combos alive, and complete the full festival run.",
      summaryRows: [
  ...
  if (snapshot.screen === "LEVEL_FAILED") {
    return {
      ...
      subtitle:
        "Too many hazard encounters ended this round. Retry now or return to menu.",
      summaryRows: [
  ...
  if (snapshot.screen === "FESTIVAL_COMPLETE") {
    return {
      ...
      summaryRows: [
        ...
        { label: "Final tier", value: formatPerformance(snapshot) },
        { label: "Best festival", value: formatScore(snapshot.profile.bestFestivalScore) }
      ],
      actions: [
